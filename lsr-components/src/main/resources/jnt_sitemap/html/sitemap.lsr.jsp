...
<c:forEach var="level1Page" items="${jcr:getChildrenOfType(base, 'jmix:sitemap')}" varStatus="level1Status">
      <c:choose>
         <c:when test="${jcr:isNodeType(level1Page, 'jnt:navMenuText')}">
            <h3><a href='' onclick="return false"><strong>${level1Page.displayableName}</strong></a></h3>
         </c:when>
         <c:otherwise>
            <h3><a href="${level1Page.url}" title="${level1Page.displayableName}"><strong>${level1Page.displayableName}</strong></a></h3>
         </c:otherwise>
      </c:choose>		
        <c:forEach var="level2Page" items="${jcr:getChildrenOfType(level1Page, 'jmix:sitemap')}" varStatus="level2Status">
            ...
            <li><i class="icon-file"></i>
              <c:choose>
                ...
                <c:otherwise>
                     <c:if test="${jcr:isNodeType(level2Page,'jnt:nodeLink')}">
						<a href="${level2Page.properties['j:node'].node.url}" >${ level2Page.displayableName}</a>
					</c:if>
                     <c:if test="${!jcr:isNodeType(level2Page,'jnt:nodeLink')}">
