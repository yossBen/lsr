...
<c:if test="${empty currentResource.moduleParams.base}">
	<c:if test="${empty baseline or baseline.string eq 'home'}">
		<c:set var="current" value="${currentPage.resolveSite.home}" />
	</c:if>
</c:if>
...
<template:addCacheDependency path="${currentPage.parent.path}" />
...
<ul id="${currentResource.moduleParams.id}" class="${currentResource.moduleParams.class}">
	<c:forEach items="${items}" var="menuItem">
		<%--iterate over each menu item--%>
		<c:set var="inpath" value="${fn:startsWith(currentPage.path, menuItem.path)}" />
