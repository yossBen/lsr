...
<section class="bloc-actualites" role="contentinfo" aria-label="Actualités">
   ...
   <div class="actualites">
      ...
      <c:choose>
         <c:when test="${functions:length(query11.nodes)>0}">
            ...
            <jcr:sql var="query12" sql="select * from [lnt:actualite] where isdescendantnode('${currentNode.resolveSite.path}') order by [datePublication] desc" limit="${max.long-1}" />
            <c:forEach items="${query12.nodes}" var="node">
                              <jcr:nodeProperty node="${node}" name="jcr:title" var="titre" />
                              <jcr:nodeProperty node="${node}" name="image" var="image" />
                              <jcr:nodeProperty node="${node}" name="chapeau" var="chapeau" />
               <article class="actu" role="article">
   ...
   <div class="touteslesactus">
      <a href="${url.server}${pageToutesActus.node.url}" onclick="plusdactus(this.href); return false;" class="bttoutes" role="link" aria-label="Voir plus d’articles">Voir plus d’articles</a>
   </div>
</section>